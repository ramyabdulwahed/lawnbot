...
class MotorController(Node):
    ...
    def odom_update_callback(self):
        ...
        What Is This Code Doing? 
            This is your robot's odometry calculation logic. 
            Your robot asks: “How much did the left wheel rotate since last time? And the right wheel?” 
            From this, it estimates: 
                How far did I move forward? 
                Did I turn? If so, by how much? 
                Then: Where am I now (x, y, θ)?
        ...
        what the code below does:
            ...
            Asking channel 1 for position

            Getting something like "1,p1235"

            Removing the p or P

            Converting the rest into an int (ticks)
        '''
        if left_wheel_motor.startswith('1,'):
            response = left_wheel_motor.split(',')
            left_wheel = response[1]
            left_wheel = int(left_wheel[1:])  # Remove 'p' and convert to int
        else:
            self.get_logger().warn(f"Unexpected Drive reply: {left_wheel_motor}")
            return 
        

        # === STEP 2 — Query motor 2 encoder ===
        # Example expected reply: T=5678 → means turn encoder = 5678 ticks

        right_wheel_motor = send_command(self.ser, "2,getp")
        
        if right_wheel_motor.startswith('2,'):
            response = right_wheel_motor.split(',')
            right_wheel = response[1]
            right_wheel = int(right_wheel[1:])
        else:
            self.get_logger().warn(f"Unexpected Turn reply: {right_wheel_motor}")
            return  
        
          # === STEP 3 — Convert TICKS → meters and radians ===

        ticks_per_meter = 5435
        ticks_per_radian = 1178 

        # # 1. Compute how much each wheel moved since the last reading (in encoder ticks)
        delta_left_wheel_drive = left_wheel - self.last_ticks_left_wheel_drive
        delta_right_wheel_drive = right_wheel - self.last_ticks_right_wheel_drive

        # # 2.Convert ticks to meters moved (distance traveled by each wheel)
        delta_distance_left = delta_left_wheel_drive / ticks_per_meter  # meters moved forward
        delta_distance_right = delta_right_wheel_drive / ticks_per_meter  

        # 3. Store the current encoder values for the next cycle
        self.last_ticks_left_wheel_drive = left_wheel
        self.last_ticks_right_wheel_drive = right_wheel
        
        # 4. Estimate how far the robot moved forward: average of left/right distances
        delta_distance = (delta_distance_left + delta_distance_right) / 2.0  # Average distance moved

        # 5. Estimate how much the robot rotated (angular change in radians)
        #    If left and right moved equally, this is 0 (straight line). If not, it's a turn.
        delta_theta = (delta_distance_right - delta_distance_left) / self.wheel_separation

        # 6. Update robot's current heading (rotation angle in radians)
        ...
        # 7. Update robot's global x and y position using polar → Cartesian projection
